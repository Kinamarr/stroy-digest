"""
CI-friendly scraper: no browser, no input(), writes to docs/ for GitHub Pages,
...
def generate_html(all_results: list, archive_links: list, pages_url: str = '') -> str:
    ...
    for i, item in enumerate(all_results):
        card_id = f"c{i}"
        cities_html = ''.join(f'<span class="tag city">{c}</span>' for c in item['cities'])
        kw_html = ''.join(f'<span class="tag kw">{k}</span>' for k in item['keywords'][:3])
    ...
    archive_html = ''
    if archive_links:
        items = ''.join(f'<a href="{href}" class="arch-link">{label}</a>' for href, label in archive_links)
...
def post_to_bitrix(webhook: str, group_id: str, results: list, pages_url: str, date_str: str):
    """Post a summary to a Bitrix24 group feed via REST API."""
    if not webhook or not group_id:
        print('Bitrix24: переменные BITRIX_WEBHOOK / BITRIX_GROUP_ID не заданы, пропускаю.')
        return
    ...
    lines = [f'[B]🏗 Стройдайджест — {date_str}[/B]', '']
    for cat, items in sorted(by_cat.items()):
        icon = CAT_ICON.get(cat, '•')
        lines.append(f'[B]{icon} {cat}[/B] — {len(items)} объект(ов)')
        for item in items[:3]:
            short = item['text'][:120].replace('\n', ' ')
            lines.append(f'  • [{item["link"]} {short}...]')
        if len(items) > 3:
            lines.append(f'  ...и ещё {len(items) - 3}')
        lines.append('')

    if pages_url:
        lines.append(f'[URL={pages_url}]→ Открыть полный дайджест[/URL]')

    message = '\n'.join(lines)

    try:
        url = webhook.rstrip('/') + '/log.blogpost.add'
        resp = httpx.post(url, json={
            'GROUP_ID': group_id,
            'TITLE': f'Стройдайджест {date_str}',
            'TEXT': message,
        }, timeout=20)
        resp.raise_for_status()
        result = resp.json()
        if result.get('result'):
            print(f'Bitrix24: пост опубликован (id={result["result"]})')
        else:
            print(f'Bitrix24: ошибка ответа — {result}')
    except Exception as e:
        print(f'Bitrix24: ошибка публикации — {e}')
...
def main():
    ...
    archive_links = [(f'archive/{f.name}', f.stem) for f in existing[:14]]

    pages_url = os.environ.get('PAGES_URL', '')

    html = generate_html(all_results, archive_links, pages_url)

    # index.html — всегда актуальный
    (docs_dir / 'index.html').write_text(html, encoding='utf-8')
    # архив с датой
    (archive_dir / f'{date_str}.html').write_text(html, encoding='utf-8')

    print(f'Сохранено: docs/index.html и docs/archive/{date_str}.html')

    # Публикация в Bitrix24
    bitrix_webhook = os.environ.get('BITRIX_WEBHOOK', '')
    bitrix_group   = os.environ.get('BITRIX_GROUP_ID', '')
    post_to_bitrix(bitrix_webhook, bitrix_group, all_results, pages_url, date_display)
